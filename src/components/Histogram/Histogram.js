...
export default function Histogram({ x, y, width, data, axisMargin, bins, value, height, bottomMargin }) {

    const {bars, widthScale, yScale} = useD3Stuff({bins, value, data, width, axisMargin, height, bottomMargin, y});
    const N = data.length;
...
function useD3Stuff({bins, value, data, width, axisMargin, height, bottomMargin, y}) {
    const bars = useMemo(() => d3
            .histogram()
            .thresholds(bins)
            .value(value)(data),
        [bins, value, data]);
